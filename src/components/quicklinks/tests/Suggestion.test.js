...
test('renders contact us page title', () => {
    render(<Suggestion />);
    const pageTitle = screen.getByRole('heading', { name:'Suggest Feature'});
    expect(pageTitle.textContent).toBe("Suggest Feature");
});

test('renders contact us introduction correctly', () => {
    render(<Suggestion />);
    const introductionInfo = screen.getByTestId('introduction');
    expect(introductionInfo.textContent).toBe("Hello there, You are at Etour feature suggestion page. Here you can reach us under the address bellow or alternatively send as a messages concerning an upgrade or a new feature in the form bellow. Please remember to leave accurate information to enable us to reach back and server you in time.")
});
